...
class MASORT(object):
    def __init__(self, args, frame_rate=30):
        ...
        if args.with_reid:
            if args.benchmark=='MMMUAV':
                path = "pretrained/multi_uav-rgb.t7"
                self.encoder = Extractor(model_path=path, device=self.args.device, num_classes=2915)  # 2915
    ...
    def update(self, output_results, img, img_event=None):
        ...
        if len(dets) > 0:
            ...
            '''Detections'''
            if self.args.with_reid:
                detections = [STrack(STrack.tlbr_to_tlwh(tlbr), s, f, f_e, use_recent=self.use_recent) for
        ...
        if Vis:
            if dists.size > 0 and np.any(dists):
                print("final dists rgb in stage 1 = \n", dists)
...
def crop_targets_2modal(
    ...
    """
    对RGB图像和事件图像分别按照独立的增益和填充参数裁剪目标区域。
